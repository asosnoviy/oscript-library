...
Процедура ЗаполнитьНастройкиИзПараметров(знач ЗначенияПараметров)
	...
	Если ЗначенияПараметров["-usedefault"] <> Неопределено ИЛИ ЗначенияПараметров["-s"] <> Неопределено Тогда
		мНастройки.Прокси.ИспользоватьПрокси = Истина;
	КонецЕсли;

	Если ЗначенияПараметров["-usedefault"] <> Неопределено Тогда
		мНастройки.Прокси.ПроксиПоУмолчанию = Истина;
	КонецЕсли;	
	Если ЗначенияПараметров["-s"] <> Неопределено Тогда
		мНастройки.Прокси.Сервер= ЗначенияПараметров["-s"];
	КонецЕсли;	
	Если ЗначенияПараметров["-p"] <> Неопределено Тогда
		мНастройки.Прокси.Порт = ЗначенияПараметров["-p"];
	КонецЕсли;
	Если ЗначенияПараметров["-user"] <> Неопределено Тогда
		мНастройки.Прокси.Пользователь = ЗначенияПараметров["-proxy-user"];
	КонецЕсли;	
		Если ЗначенияПараметров["-pass"] <> Неопределено Тогда
		мНастройки.Прокси.Пароль = ЗначенияПараметров["-proxy-pas"];
	КонецЕсли;	
